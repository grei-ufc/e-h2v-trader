...
def plot_scenario_results(TIME_HORIZON, s_block, s_name, initial_soc_dict, fixed_h2_price, fixed_ammonia_price, 
    ...
    # --- EXTRAÇÃO DE RESULTADOS (ATUALIZADA PARA MODELO LINEAR) ---
    ...
    # CORREÇÃO DO ERRO: Somamos as duas variáveis que substituíram ELECTROLYZER_ELEC_CONSUMPTION
    ...
    # Consumo elétrico da planta de amônia
    ...
    fig.suptitle(f'Resultados da Operação - Cenário: {s_name.upper()}', fontsize=16)

    # 1. Balanço de Potência
    ax1.plot(TIME_HORIZON, res_solar, label='Solar', color='gold')
    ax1.plot(TIME_HORIZON, res_wind, label='Eólica', color='skyblue')
    ax1.plot(TIME_HORIZON, res_grid_buy, label='Compra Rede', color='orange')
    ax1.plot(TIME_HORIZON, -res_load, label='Carga', color='red', linestyle='--')
    ax1.plot(TIME_HORIZON, -res_electrolyzer_consumption, label='Eletrolisador (PEM)', color='magenta')
    ax1.plot(TIME_HORIZON, -res_ammonia_plant_elec_mw, label='Planta Amônia', color='brown')
    ax1.plot(TIME_HORIZON, -res_grid_sell, label='Venda Rede', color='navy')
    ...
    ax2_pld = ax2.twinx()
    ax2_pld.plot(TIME_HORIZON, res_elec_price, color='green', linestyle=':', label='PLD (R$/MWh)')
    ...
    ax3.legend(lines3 + lines4, labels3 + labels4, loc='upper left', bbox_to_anchor=(1.05, 1))
